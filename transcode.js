...
function transcodeAndMoveFile(arrayIndex){
    ...
    child_process.exec('for f in '+path+'; do ffmpeg -i "$f" -f mp4 -vcodec libx264 -preset medium -acodec aac -movflags faststart -vf scale=-1:720,format=yuv420p transcoded/'+fileName+'.mp4; mv "$f" "$f".encoded ; done',
